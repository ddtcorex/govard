...
- **Dagster Framework Support:** Govard now fully supports Dagster projects: its own runtime config (configurable Python version, no PHP container), `workspace.yaml`/`dagster.yaml`-based detection, a compose blueprint (`dagster dev` web service + PostgreSQL run/event-log storage), and both a fresh-install bootstrap (`dagster project scaffold` inside a disposable container, with `dagster.yaml`/`workspace.yaml` written against the scaffolded project's real Definitions module) and a clone bootstrap (`pip install` against the project's own `requirements.txt`).

...
func MagentoConfigCommandsForTest(projectName string, config engine.Config) []magentoCommand {
...
func ConfigureMagento(projectName string, config engine.Config, force bool, shiftInfo *engine.ProfileShiftInfo) error {
	shifted := force
	...
	if shiftInfo != nil {
		...
		shifted, reason = engine.CheckProfileShiftCleanup(config)
	}
	...
	frameworkName := displayNameForDistribution(config.Framework)
...
func buildFrameworkAutoConfigurationCommands(projectName string, config engine.Config) []magentoCommand {
...
func buildMagento2Commands(projectName string, config engine.Config, lockedKeys map[string]bool) []magentoCommand {
	containerName := fmt.Sprintf("%s%s", projectName, conventions.PHPSuffix)
	...
	if tablePrefix := engine.NormalizeTablePrefix(config.TablePrefix); tablePrefix != "" {
...
func buildMagento1Commands(projectName string, config engine.Config) []magentoCommand {
...
func buildMagentoSearchConfigSetCommands(containerName string, config engine.Config, engineName string) []magentoCommand {
...
func ResolveMagentoSearchEngine(config engine.Config) string {
...
func isMagentoVersionAtLeast(raw string, minimum string) bool {
	return engine.IsNumericDotVersionAtLeast(raw, minimum)
}
...
func magentoDockerExecArgs(containerName string, config engine.Config, args ...string) []string {
	result := []string{"exec"}
...
func magento1DockerSQLExecArgs(containerName string, sql string) []string {
	script := fmt.Sprintf(
		...
		engine.ShellQuote(sql), engine.ShellQuote(conventions.DefaultMagentoDBUser), engine.ShellQuote(conventions.DefaultMagentoDBName),
	)
...
func resolveMagentoExecUser(config engine.Config) string {
...
func FixProjectPermissions(projectName string, config engine.Config) error {
	...
	for _, d := range config.Stack.ChownDirList {
		dirs += fmt.Sprintf("%s ", engine.ShellQuote(d))
	}
...
func CheckMagentoEnvPHPLockedKeys(containerName string, config engine.Config) (map[string]bool, error) {
...
func SetMagentoComposerInstallRunnerForTest(fn func(projectName string, config engine.Config, stdout, stderr io.Writer) error) func() {
...
func MaybeRunMagentoComposerInstallForTest(projectName string, config engine.Config) {
	maybeRunMagentoComposerInstall(projectName, config)
}
...
func maybeRunMagentoComposerInstall(projectName string, config engine.Config) {
	skipComposerInstall := false
	if projectRoot, rootErr := os.Getwd(); rootErr == nil {
		skipComposerInstall, _ = engine.VendorSatisfiesComposerLock(projectRoot)
	}
...
func runMagentoComposerInstall(projectName string, config engine.Config, stdout, stderr io.Writer) error {
	containerName := fmt.Sprintf("%s%s", projectName, conventions.PHPSuffix)
...
func backupMagentoComposerRuntime(containerName string, config engine.Config) {
...
func restoreMagentoComposerRuntimeBackup(containerName string, config engine.Config) {
...
func discardMagentoComposerRuntimeBackup(containerName string, config engine.Config) {
...
func PrepareRunMappingAssets(config engine.Config) (string, string, error) {
	if strings.TrimSpace(config.ProjectName) == "" {
		return "", "", nil
	}

	nginxPath := filepath.Join(engine.GovardHomeDir(), "nginx", config.ProjectName, "mage-run-map.conf")
	apachePath := filepath.Join(engine.GovardHomeDir(), "apache", config.ProjectName, "mage-run-map.conf")
...
func buildMagentoNginxRunMap(mappings engine.StoreDomainMappings) string {
	lines := []string{
		"map $host $mage_run_code {",
		`    default "";`,
	}

	typedHosts := engine.SortedStoreDomainHosts(mappings)
...
func buildMagentoApacheRunMap(mappings engine.StoreDomainMappings) string {
	lines := []string{"# Generated by Govard"}
	for _, host := range engine.SortedStoreDomainHosts(mappings) {
		mapping := mappings[host]
		if mapping.ScopeType() == "" || mapping.ScopeCode() == "" {
			continue
		}
...
// CheckProfileShiftCleanupForTest exposes engine.CheckProfileShiftCleanup for testing.
func CheckProfileShiftCleanupForTest(config engine.Config) (bool, string) {
	return engine.CheckProfileShiftCleanup(config)
}

// DetectProfileShiftForTest exposes engine.DetectProfileShift for testing.
func DetectProfileShiftForTest(config engine.Config) engine.ProfileShiftInfo {
	return engine.DetectProfileShift(config)
}

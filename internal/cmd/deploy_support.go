...
func bindDeployFlags(command *cobra.Command) {
	...
	command.Flags().Int("keep", 0, "How many releases to keep on the target (0 means the remote keep_releases setting, default 5)")
	...
	command.Flags().Bool("resume", false, "Continue the newest unfinished release instead of starting a new one (starts a new release with a notice when nothing is unfinished)")

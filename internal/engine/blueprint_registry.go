...
func resolveBlueprintRegistryBlueprintsDir(cfg BlueprintRegistryConfig) (string, error) {
	...
	if err := os.MkdirAll(cacheRoot, conventions.SecretDirPerm); err != nil {
	...
	if err := os.MkdirAll(stagedCacheDir, conventions.DefaultDirPerm); err != nil {
...
func extractBlueprintArchive(payload []byte, workspace string) (string, error) {
	...
	if err := os.MkdirAll(tarRoot, conventions.SecretDirPerm); err != nil {
	...
	if err := os.MkdirAll(zipRoot, conventions.SecretDirPerm); err != nil {
...
func extractTarGzArchive(payload []byte, dest string) error {
	...
	for {
		...
		case tar.TypeReg:
			...
			file, err := os.OpenFile(targetPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, conventions.DefaultFilePerm)
			if err != nil {
				return err
			}
...
func extractZipArchive(payload []byte, dest string) error {
	...
	for _, zipped := range reader.File {
		...
		target, err := os.OpenFile(targetPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, conventions.DefaultFilePerm)
		if err != nil {
			_ = source.Close()
			return err
		}
...
func copyDirectory(src, dst string) error {
	return filepath.WalkDir(src, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		if entry.IsDir() {
			return os.MkdirAll(target, conventions.DefaultDirPerm)
		}
		...
		if err := os.MkdirAll(filepath.Dir(target), conventions.DefaultDirPerm); err != nil {
			return err
		}
		return os.WriteFile(target, data, conventions.DefaultFilePerm)
	})
}

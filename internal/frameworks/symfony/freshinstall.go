...
// freshInstall runs Symfony's fresh-install sequence. Symfony has no
...
// map was `"symfony": {needsDB: true}`), so it just delegates to the
...
func freshInstall(opts bootstrap.Options, projectDir string, helpers bootstrap.CmdHelpers) error {
	return bootstrap.GenericFreshInstall(NewSymfonyBootstrap(opts), projectDir, helpers)
}

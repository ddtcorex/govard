...
// UpgradeVariant parameterizes the shared Magento-2-family upgrade
...
var defaultUpgradeVariant = UpgradeVariant{
...
// Upgrade is magento2's engine.UpgradeFunc - registered via
// FrameworkDefinition.Upgrade in Definition().
func Upgrade(ctx context.Context, config engine.Config, opts engine.UpgradeOptions) error {
	return RunUpgrade(ctx, config, opts, defaultUpgradeVariant)
}

func RunUpgrade(ctx context.Context, config engine.Config, opts engine.UpgradeOptions, variant UpgradeVariant) error {
	...
	if !opts.NoEnvUpdate {
		...
		targetProfile, err := engine.ResolveRuntimeProfile(config.Framework, opts.TargetVersion)
		if err != nil {
			pterm.Warning.Printf("Could not resolve specific profile for %s (continuing): %v\n", opts.TargetVersion, err)
		} else {
			engine.ApplyRuntimeProfileToConfig(&config, targetProfile.Profile)
			engine.NormalizeConfig(&config, opts.ProjectDir)
			// Ensure it writes to file
			cleanConfig := engine.PrepareConfigForWrite(config)
			...
			if err := engine.RenderBlueprint(opts.ProjectDir, config); err != nil {
		...
		composePath := engine.ComposeFilePathWithProfile(opts.ProjectDir, opts.ProjectName, config.Profile)
		if err := engine.RunCompose(ctx, engine.ComposeOptions{
	...
	if err := engine.FixComposerCompatibility(config); err != nil {
...
func updateMagentoComposerJson(opts engine.UpgradeOptions, containerName string, variant UpgradeVariant) error {
...
func checkDatabaseReady(ctx context.Context, config engine.Config, containerName string) {
	for i := 0; i < 30; i++ {

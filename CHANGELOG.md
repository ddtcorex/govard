...
- **PrestaShop Support:** Govard now auto-detects PrestaShop projects and applies framework-specific runtime defaults (PHP 8.1, MariaDB 10.11, no forced cache/search/queue). Adds a dedicated nginx blueprint template handling PrestaShop's friendly-URL product/category image rewrites, plus Docker Compose service wiring. Bootstrap supports detecting and cloning/configuring an existing PrestaShop install (no fresh-install pipeline yet). `govard tool prestashop [command]` runs the PrestaShop CLI inside the PHP container, and remote DB credential sync works the same as other supported frameworks.

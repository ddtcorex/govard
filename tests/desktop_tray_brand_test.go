...
// must call both to read "Govard Desktop" on every platform.
func TestDesktopTrayIsBrandedThroughLabelAndTooltip(t *testing.T) {
	tray := &recordingTray{}
	desktop.BrandTrayForTest(tray)
	if tray.label != "Govard Desktop" {
		t.Fatalf("label = %q, want %q (the SNI Title on Linux)", tray.label, "Govard Desktop")
	}
	if tray.tooltip != "Govard Desktop" {
		t.Fatalf("tooltip = %q, want %q", tray.tooltip, "Govard Desktop")
	}
}
